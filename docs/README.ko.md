...
이 프로젝트는 Codex/Qwen/Ollama/OpenRouter/Groq + Telegram 연동에 집중합니다.  
...
온보딩 실행(대화형 provider 설정: Codex/Qwen은 OAuth, Ollama는 endpoint 입력, OpenRouter는 API 키 + 무료 모델 스캔, Groq는 API 키 + 모델 스캔):
...
- Groq: API 키 + base URL(기본값 `https://api.groq.com/openai/v1`) 입력 후 모델을 동적으로 스캔/선택
...
- `/provider <id|alias|번호>`: provider 전환(Codex/Qwen은 OAuth, Ollama는 endpoint 입력 안내, OpenRouter는 API 키 입력 + 무료 모델 스캔, Groq는 API 키 입력 + 모델 스캔)
...
- Groq 설정 진행 중에는 다음 일반 메시지를 Groq API 키 입력으로 처리
